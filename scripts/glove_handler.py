...
class GloveHandler:
    ...
    def get_all_terms(self):
        sql = "select term from glove_vecs"
        cursor = self.conn.cursor()
        cursor.execute(sql)
        terms = []
        for row in cursor:
    ...
    def get_glove_vec(self, term):
        ...
        if vec_blob:
            ...
            arr.frombytes(vec_blob[0])
            glove_vec = arr.tolist()
        else:
            return None
        return glove_vec

def build_wp_glove_vector_db():
    home = expanduser("~")
    glove_file = '/ws/tools/glove/pa_2020_bpe_glove_vectors.txt'
    db_file = home + "/pa_2020_bpe_glove_vectors.db"
